...
int main (int argc, char **argv) {
    ...
    while(!stop_main_loop) {
        ...
        //     clock_gettime(CLOCK_MONOTONIC, &start_time);
            ...
            if (settings.use_udp == 1) {
                ...
                SelfContained(2000000, settings.key_size, settings.value_size, settings.vsize_skew,
                                        settings.vsize_min, settings.vsize_max, settings.ksize_skew, settings.ksize_min, 
                                        settings.ksize_max, 2000000, settings.kMaxValSize, settings.use_udp, true);
                SelfContained(2000000, settings.key_size, settings.value_size, settings.vsize_skew,
                                        settings.vsize_min, settings.vsize_max, settings.ksize_skew, settings.ksize_min, 
                                        settings.ksize_max, 2000000, settings.kMaxValSize, settings.use_udp, false);
            return;
                // count++;
        ...
        // count = 0; // Reset count
        // clock_gettime(CLOCK_MONOTONIC, &start); // Reset the start time
        // }
